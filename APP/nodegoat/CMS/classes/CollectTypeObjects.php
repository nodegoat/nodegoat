...
class CollectTypeObjects {
	...
	public function init($arr_filter = [], $collect = true) {
		
		$this->arr_type_filters = $this->arr_types = [];
		
		$this->arr_path_objects = $this->arr_path_objects_referenced = [];
		
		$this->collectObjects($this->arr_type_network_paths, $arr_filter, false, $collect);
	...
	public function getPathObject($path, $in_out, $object_id, $ref_object_id) {
		...
		foreach ($arr_object['object_subs'] as $object_sub_id => &$arr_object_sub) {
			...
			if ($this->arr_path_object_connections[$path]['object_sub_details'][$object_sub_details_id]['use'] == 'solo' && !$this->arr_path_objects_referenced[$path][$ref_object_id]['object_subs'][$object_sub_details_id][$object_sub_id]) {
					...
					if ($arr_type_object_connections['object_descriptions']) {
						...
						foreach ($arr_type_object_connections['object_descriptions'] as $object_description_id => $arr_connection) {
							...
							if ($in_out == 'out') {
								...
								$arr_ref_object_ids = (array)$this->arr_path_objects_referenced[$path_new][$object_id]['object_definitions'][$object_description_id];
							...
							foreach ($arr_object_sub_connections as $object_sub_connection_id => $arr_connection) {
								...
								if ($object_sub_connection_id === 'object_sub_location') {
									...
									if ($in_out == 'out') {
										...
										$arr_object_subs = $this->arr_path_objects_referenced[$path_new][$object_id]['object_subs'][$object_sub_details_id];
	...
	private function collectObjects($arr_type_connections, $arr_source_filters, $filter_source, $collect = true) {
		...
		foreach ($arr_type_connections as $in_out => $arr_in_out) {
			...
			foreach ($arr_in_out as $target_type_id => $arr_type_object_connections) {
				...
				if ($this->arr_path_objects[$path_new][$in_out] === null) {
					$this->arr_path_objects[$path_new][$in_out] = [];
				}
				
				if ($collect && $this->is_generating) {
					
					$has_objects = false;
					
					foreach ($this->arr_path_generate[$path_new][$in_out] as $filter) {
					...
					if ($has_objects && $arr_type_connections['connections'][$target_type_id]) {
					...
					if ($collect) {
						...
						$filter = new FilterTypeObjects($target_type_id, $this->view);
					...
					if ($in_out != 'start') {
						...
						foreach ((array)$arr_type_object_connections['object_descriptions'] as $object_description_id => $arr_connection) {
							...
							if ($in_out == 'out') {
								...
								if ($filter_source) {
									...
									$s_arr = &$arr_path_filter['object_filter'][]['referenced_types'][$arr_connection['type_id']]['object_definitions'][$object_description_id];
								...
								$s_arr = &$arr_path_filter['object_filter'][]['object_definitions'][$object_description_id];
						...
						foreach ((array)$arr_type_object_connections['object_sub_details'] as $object_sub_details_id => $arr_object_sub_connections) {
							...
							foreach ($arr_object_sub_connections as $object_sub_connection_id => $arr_connection) {
								...
								if ($object_sub_connection_id === 'object_sub_location') {
									...
									if ($in_out == 'out') {
										...
										$s_arr = &$arr_path_filter['object_filter'][]['referenced_types'][$arr_connection['type_id']]['object_subs'][$object_sub_details_id]['object_sub_location_reference'];
										...
										$s_arr = &$arr_path_filter['object_filter'][]['object_subs'][$object_sub_details_id]['object_sub_locations'][];
										...
										if ($filter_source) {
											// Already in filter (as object query), but do filter the subobjects if they are not needed as a followup connection
											if ($arr_source_filters && !$arr_type_connections['connections'][$target_type_id]['out'][$arr_connection['ref_type_id']]['object_sub_details'][$object_sub_details_id]['object_sub_location']) {
										...
										if ($arr_selection && !$arr_selection['object_sub_details'][$object_sub_details_id]['object_sub_details']) {
											
											$arr_selection_required['object_sub_details'][$object_sub_details_id]['object_sub_details'] = ['object_sub_location_reference' => true];
											
											if (!$arr_selection_required['object_sub_details'][$object_sub_details_id]['object_sub_descriptions']) {
									...
									if ($in_out == 'out') {
										...
										$s_arr = &$arr_path_filter['object_filter'][]['referenced_types'][$arr_connection['type_id']]['object_subs'][$object_sub_details_id]['object_sub_definitions'][$object_sub_connection_id];
										...
										$s_arr = &$arr_path_filter['object_filter'][]['object_subs'][$object_sub_details_id]['object_sub_definitions'][$object_sub_connection_id];
										
										if ($filter_source) {
											// Already in filter (as object query), but do filter the subobjects if they are not needed as a followup connection
											if ($arr_source_filters && !$arr_type_connections['connections'][$target_type_id]['out'][$arr_connection['ref_type_id']]['object_sub_details'][$object_sub_details_id][$object_sub_connection_id]) {
												$s_arr[]['query_dependent'] = $filter_source->format2SQLObject();
											}
										} else {
											if ($arr_source_filters) {
								...
								$cur_use = $this->arr_path_object_connections[$path_new]['object_sub_details'][$object_sub_details_id]['use'];
								
								if ($cur_use != 'all') {
										
									$this->arr_path_object_connections[$path_new]['object_sub_details'][$object_sub_details_id]['use'] = ($in_out == 'in' ? 'solo' : 'all');
									
									// When the subobject is used as both in/referenced and out in the same path, all subobjects are available to generate the path
									if ($this->arr_path_object_connections[$path_source]['object_sub_details'][$object_sub_details_id]['object_sub_descriptions'][$object_sub_connection_id]) {
					...
					if ($arr_selection) { // Collect values from referenced objects originally not in selection but needed for collection
						...
						foreach ((array)$arr_type_connections['connections'][$target_type_id]['out'] as $user_type_id => $arr_target_type_object_connections) {
					...
					foreach ((array)$this->arr_limit_type_filters[$target_type_id] as $arr_limit_type_filter) {
					...
					if ($filter) {
						...
						if ($in_out == 'start') {
							...
							$filter_depth = new FilterTypeObjects($target_type_id, $this->view);
							...
							if ($arr_filtering_enable) {
								...
								foreach ((array)$arr_depth_filter as $arr_filter) {
						...
						$limit = ($this->arr_path_options[$path_new]['limit'] ?: ($this->arr_type_options[$target_type_id]['limit'] ?: false));
						...
						if ($this->conditions !== null) {
							...
							if (!$this->conditions) {
								...
								$this->arr_conditions_found[serialize($arr_conditions)] = $arr_conditions;

								$filter->setConditions($this->conditions, $arr_conditions);
							}
						}
					...
					if ($arr_type_connections['connections'][$target_type_id]) {
						$this->collectObjects($arr_type_connections['connections'][$target_type_id], $arr_collect_filters['all'], $filter, $collect);
					}
				}
			}
		}
	...
	private function collectFilters($arr_type_connections) {
		...
		foreach (['start' => (array)$arr_type_connections['start'], 'in' => (array)$arr_type_connections['in'], 'out' => (array)$arr_type_connections['out']] as $in_out => $arr_in_out) {
			foreach ($arr_in_out as $target_type_id => $arr_type_object_connections) {
				
				$arr_source_filters = $this->collectFilters($arr_type_connections['connections'][$target_type_id]);
				
				foreach ((array)$this->arr_filter_type_filters[$target_type_id] as $arr_filter) {
