...
class CollectTypeObjectsValues extends CollectTypeObjects {
	
	protected $type_id = false;
	protected $value_type = '';
	protected $arr_value_type_options = [];
    ...
    public function __construct($type_id, $view = 'set', $value_type = '', $arr_value_type_options = []) {
		
		$this->type_id = $type_id;
		$this->value_type = $value_type;
		$this->arr_value_type_options = $arr_value_type_options;
	...
	public function prepare($arr_type_network, $arr_filters) {
		...
		if ($arr_type_network['paths']) {
			...
			$trace->filterTypeNetwork($arr_type_network['paths']);
		...
		foreach ($arr_collect_info['types'] as $cur_type_id => $arr_paths) {
			...
			if ($this->arr_scope['project_id']) {
				...
				$arr_project = cms_nodegoat_custom_projects::getProjects($this->arr_scope['project_id']);
			...
			foreach ($arr_objects as $object_id => $arr_object) {
				...
				if ($arr_walked) {
					...
					if ($arr_walked['value']) {
						...
						if ($this->arr_value_type_options['int_direction'] == StoreType::TIME_BEFORE_END || $this->arr_value_type_options['int_direction'] == StoreType::TIME_AFTER_END) {
					...
					if ($arr_walked['object_sub_ids']) {
						...
						foreach ($arr_walked['object_sub_ids'] as $object_sub_id => $value_object_sub) {
							...
							$arr_sql_insert[] = "(".(int)$this->arr_value_type_options['int_identifier'].", ".$object_id.", ".$object_sub_id.", ".(int)$value_object_sub.")";
						}
					} else if ($value_object) {
						
						$arr_sql_insert[] = "(".(int)$this->arr_value_type_options['int_identifier'].", ".$object_id.", 0, ".(int)$value_object.")";
					}
				}
			}
