...
class CreateObjectsPackage {
	...
	private function parseObjects($type_id, $arr_objects) {
	...
	private function parseObjectsLD($type_id, $arr_objects) {
		...
		$type_name_ld = ($arr_type_schema['type']['name'] ?: $context.preg_replace('/[^a-z0-9]+/', '_', strtolower($arr_type_set['type']['name'])));
		...
		foreach ($arr_objects as $object_id => $arr_object) {
			...
			foreach ($arr_object['object_definitions'] as $object_description_id => $arr_object_definition) {
				...
				$object_description_name_ld = ($arr_schema_data_type['object_description_name'] ?: $type_name_ld.'/'.preg_replace('/[^a-z0-9]+/', '_', strtolower($arr_object_description['object_description_name'])));
				...
				if ($arr_object_definition['object_definition_ref_object_id']) {
					...
					foreach ((array)$arr_object_definition['object_definition_ref_object_id'] as $key => $object_definition_ref_object_id) {
						...
						if ($arr_object_description['object_description_value_type'] == 'external') {
							...
							$str_ref_name_ld = ($arr_types_schema[$ref_type_id]['type']['name'] ?: $context.preg_replace('/[^a-z0-9]+/', '_', strtolower($arr_ref_type_set['type']['name'])));
			...
			foreach ($arr_object['object_subs'] as $object_sub_id => $arr_object_sub) {
				...
				$arr_object_sub_details = $arr_type_set['object_sub_details'][$arr_object_sub['object_sub']['object_sub_details_id']];
				$arr_object_sub_details_schema_data_type = $arr_type_schema['object_sub_details'][$arr_object_sub['object_sub']['object_sub_details_id']];
				
				$object_sub_name_ld = ($arr_object_sub_details_schema_data_type['object_sub_details']['object_sub_details_name'] ?: $type_name_ld.'/'.preg_replace('/[^a-z0-9]+/', '_', strtolower($arr_object_sub_details['object_sub_details']['object_sub_details_name'])));
				...
				if ($arr_object_sub['object_sub']['object_sub_date_start']) {
					...
					$arr_object_sub_ld[$str_name_ld] = [
						'@type' => 'schema:Date', 
						'@value' => StoreTypeObjects::int2DateStandard($arr_object_sub['object_sub']['object_sub_date_start'])
					];
				}
				...
				if ($arr_object_sub['object_sub']['object_sub_date_end']) {
					...
					$arr_object_sub_ld[$str_name_ld] = [
						'@type' => 'schema:Date', 
						'@value' => StoreTypeObjects::int2DateStandard($arr_object_sub['object_sub']['object_sub_date_end'])
					];
				}
				...
				if ($arr_object_sub['object_sub']['object_sub_location_geometry']) {
					...
					$arr_object_sub_ld[$str_name_ld] = [
						...
						'@value' => $arr_object_sub['object_sub']['object_sub_location_geometry']
					];
				}

				if ($arr_object_sub['object_sub']['object_sub_location_ref_object_id']) {
					
					$ref_type_id = $arr_object_sub['object_sub']['object_sub_location_ref_type_id'];
					...
					$str_ref_name_ld = ($arr_types_schema[$ref_type_id]['type']['name'] ?: $context.preg_replace('/[^a-z0-9]+/', '_', strtolower($arr_location_ref_type_set['type']['name'])));
					
					$arr_object_sub_ld[$str_name_ld] = [
						'@type' => $str_ref_name_ld, 
						'@id' => $base_path.GenerateTypeObjects::encodeTypeObjectId($ref_type_id, $arr_object_sub['object_sub']['object_sub_location_ref_object_id'])
					];
					...
					$arr_object_sub_ld[$str_name_ld] = [
						'@type' => 'schema:Text', 
						'@value' => $arr_object_sub['object_sub']['object_sub_location_ref_object_name']
					];
				}
				...
				foreach ($arr_object_sub['object_sub_definitions'] as $object_sub_description_id => $arr_object_sub_definition) {
					...
					$object_sub_description_name_ld = ($arr_schema_data_type['object_sub_description_name'] ?: $object_sub_name_ld.'/'.preg_replace('/[^a-z0-9]+/', '_', strtolower($arr_object_sub_description['object_sub_description_name'])));
	...
	private function formatToSchemaDataType($value_type, $value) {
		...
		switch ($value_type) {
			...
			case 'date':
				$cast_type = 'Date';
				$cast_value = StoreTypeObjects::int2DateStandard($value);
				break;
			case 'media':
	...
	private function applySchemaDataType($arr_schema_data_type, $value) {
				
		$type = $arr_schema_data_type['type'];
	
		if ($arr_schema_data_type['cast_value']) {
			
			$value = $arr_schema_data_type['cast_value']($value);
		}
