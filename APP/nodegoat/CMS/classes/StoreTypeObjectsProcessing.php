...
class StoreTypeObjectsProcessing extends StoreTypeObjects {
	...
	public static function cacheTypeObjectSubs($date_after = false, $date_to = false) {
		...
		$sql_collect = "
			-- Duplicate the nodegoat_tos_loc_changed table to include other sub-objects in its path.
					...
					(
						SELECT
							...
							".$table_name_tos.".status
								...
								".$arr_source['tables']."
							".($is_null ? "WHERE ".$arr_source['column_date']." IS NOT NULL" : "")."
					)
			...
			foreach ($filter_categories->init() as $category_id => $arr_category) {
				...
				foreach ($arr_category['object_filters'] as $ref_type_id => $arr_object_type_filter) {
					...
					try {
						...
						error('StoreTypeObjectsProcessing::setReversals ERROR:'.PHP_EOL
