...
<xsl:template name="css.style.single">
    ...
    <xsl:variable name="nameclean" select="translate($name, ' ', '')" />
    <xsl:variable name="valueclean" select="translate($value, ' ', '')" />
    <xsl:if test="$nameclean != '' and $valueclean != ''">
		<xsl:choose>
			<xsl:when test="$nameclean = 'text-decoration'">
				<xsl:attribute name="{concat('style:text-', $valueclean, '-style')}">solid</xsl:attribute>
                <xsl:attribute name="{concat('style:text-', $valueclean, '-width')}">auto</xsl:attribute>
                <xsl:attribute name="{concat('style:text-', $valueclean, '-color')}">font-color</xsl:attribute>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="{concat('fo:', $nameclean)}">
					<xsl:value-of select="$valueclean"/>
				</xsl:attribute>
			</xsl:otherwise>
		</xsl:choose>
    </xsl:if>
