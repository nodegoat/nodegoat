...
class TraceTypeNetwork {
    ...
    public function __construct($arr_type_ids = [], $dynamic = false, $object_sub_locations = false) {
		...
		$this->stmt_object['in'] = DB::prepare("SELECT
			...
			WHERE nodegoat_to_des.ref_type_id = ".DBStatement::assign('id', 'i')."
				...
				".($dynamic ? "AND NOT (nodegoat_to_des.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_to_des.ref_type_id AND mode = 1))" : "")."
		...
		$this->stmt_object_sub['in'] = DB::prepare("SELECT
			...
			WHERE CASE
				...
				".($dynamic ? "AND NOT (nodegoat_tos_des.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_tos_des.ref_type_id AND mode = 1))" : "")."
		...
		if ($dynamic) {
			...
			$this->stmt_object_dynamic['in'] = DB::prepare("SELECT
				...
				WHERE (
						...
						OR (nodegoat_to_des.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_to_des.ref_type_id AND mode = 1))
					)
					".($sql_type_ids ? "AND nodegoat_to_des.type_id IN (".$sql_type_ids.")" : "")."
					AND EXISTS (
						SELECT TRUE
			...
			$this->stmt_object_dynamic['out'] = DB::prepare("SELECT
				nodegoat_t.id AS ref_type_id, nodegoat_to_des.id AS object_description_id
					FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." nodegoat_to_des
					LEFT JOIN ".DB::getTable('DEF_NODEGOAT_TYPES')." nodegoat_t ON (
				...
				WHERE (
					...
					AND EXISTS (
						SELECT TRUE
							FROM ".DB::getTable('DATA_NODEGOAT_TYPE_OBJECT_DEFINITION_OBJECTS')." nodegoat_to_def_ref
							JOIN ".DB::getTable('DATA_NODEGOAT_TYPE_OBJECTS')." nodegoat_to ON (nodegoat_to.id = nodegoat_to_def_ref.object_id AND ".$version_select_to.")
						WHERE nodegoat_to_def_ref.object_description_id = nodegoat_to_des.id
							AND ".($sql_type_ids ? "nodegoat_to_def_ref.ref_type_id IN (".$sql_type_ids.")" : "nodegoat_to_def_ref.ref_type_id")."
			...
			$this->stmt_object_sub_dynamic['in'] = DB::prepare("SELECT
				...
				WHERE (
						...
						OR (nodegoat_tos_des.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_tos_des.ref_type_id AND mode = 1))
					)
					".($sql_type_ids ? "AND nodegoat_tos_det.type_id IN (".$sql_type_ids.")" : "")."
					AND EXISTS (
						SELECT TRUE
				...
				WHERE (
					...
					AND EXISTS (
						SELECT TRUE
							...
							JOIN ".DB::getTable('DATA_NODEGOAT_TYPE_OBJECT_SUBS')." nodegoat_tos ON (nodegoat_tos.id = nodegoat_tos_def_ref.object_sub_id AND ".$version_select_tos.")
							JOIN ".DB::getTable('DATA_NODEGOAT_TYPE_OBJECTS')." nodegoat_to ON (nodegoat_to.id = nodegoat_tos.object_id AND ".$version_select_to.")
						WHERE nodegoat_tos_def_ref.object_sub_description_id = nodegoat_tos_des.id
							AND ".($sql_type_ids ? "nodegoat_tos_def_ref.ref_type_id IN (".$sql_type_ids.")" : "nodegoat_tos_def_ref.ref_type_id")."
					...
					WHERE nodegoat_tos_det.location_ref_type_id_locked = FALSE
						...
						AND EXISTS (
							SELECT TRUE
								...
								JOIN ".DB::getTable('DATA_NODEGOAT_TYPE_OBJECTS')." nodegoat_to2 ON (nodegoat_to2.id = nodegoat_tos.location_ref_object_id AND ".$version_select_to2.")
							WHERE nodegoat_tos.object_sub_details_id = nodegoat_tos_det.id AND ".$version_select_tos."
								AND ".($sql_type_ids ? "nodegoat_tos.location_ref_type_id IN (".$sql_type_ids.")" : "nodegoat_tos.location_ref_type_id")."
						)
				...
				foreach ($arr_object_connections as &$arr_object_connection) {
					...
					if ($arr_object_connection['object_description_id']) {
						...
						if ($this->arr_prefilter_object_connections) {
							...
							$arr_prefilter = $this->arr_prefilter_object_connections[$path]['object_descriptions'][$arr_object_connection['object_description_id']];
						...
						if ($this->arr_prefilter_object_connections) {
							...
							$path = implode('-', $cur_list['path']);
							$arr_prefilter = $this->arr_prefilter_object_connections[$path]['object_sub_descriptions'][$arr_object_connection['object_sub_description_id']];
						...
						if ($this->arr_prefilter_object_connections) {
							...
							$path = implode('-', $cur_list['path']);
							$arr_prefilter = $this->arr_prefilter_object_connections[$path]['object_sub_locations'][$arr_object_connection['object_sub_details_id']];
	...
	public function filterTypeNetwork($arr_object_connections) {
